...
tspacing_arr = [ (0.25 / m) * TEMP for m in mu_arr]
...
for q in range(len(mu_arr)):
    ...
    num_trajs = len(t)
    if (len(t[0]) == 0): # If there are no trajectories for the paramters given then continue
        continue

    valid_mus.append(mu_arr[q])
    tspacing = tspacing_arr[q]
...
velvmuax.errorbar(np.array(valid_mus) / TEMP, vel_v_mu, yerr=new_vel_v_err, color=color, marker='o',  linewidth=3.0, markersize=12 ) 
...
velvmuax.yaxis.set_major_locator(ticker.FixedLocator([1.0, 3.0, 5.0, 7.0, 9.0]))
velvmuax.set_xlim(0.0, 1.05)
velvmuax.set_ylim(0.75, 10.0)
velvmufig.savefig("SubFigD/Fig4d.png")

with open("SubFigD/Fig4d.txt",'w') as f:
